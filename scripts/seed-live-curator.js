#!/usr/bin/env node
/**
 * seed-live-curator.js
 *
 * Creates a placeholder museum curator account in the LIVE Firebase project
 * and seeds all curated songs + submissions under that account.
 *
 * Run this once to bootstrap production until the real curator account is set up.
 * Delete or replace the placeholder user once the real curator signs in.
 *
 * Prerequisites:
 ...
 *
 * Usage:
 *   node scripts/seed-live-curator.js
 */
...
async function getOrCreateCurator() {
  ...
  console.log(`Created placeholder curator and granted admin claim (uid: ${user.uid})`);
  return user;
}
...
async function seed() {
  ...
  await batch.commit();
  console.log(`Seeded ${songs.length} curated songs and ${songs.length} submissions into production.`);
  console.log('Remember to delete the placeholder curator@museum.local account once the real curator is set up.');
}
